#!/bin/bash
# Install KServe + Knative + Istio + cert-manager onto the serving cluster.
#
# Ported from the reference `serving-cluster/deploy_kserve.sh`; versions and
# flow kept identical. Run against the serving kubeconfig (see
# kubeconfig-serving.yaml placeholder):
#
#   KUBECONFIG=infra/serving-cluster/kubeconfig-serving.yaml \
#       bash infra/serving-cluster/deploy_kserve.sh
#
# Free-tier EKS note: KServe itself is fine on CPU. The Triton InferenceService
# (inferenceservice-triton.yaml) is pinned to CPU resources to fit the free
# tier — no GPU node group required.
set -e

# Wait for every Pod in a namespace to be Ready.
wait_for_pods_ready() {
    NS=$1
