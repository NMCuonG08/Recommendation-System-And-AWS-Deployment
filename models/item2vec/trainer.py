"""LightningModule wrapping the SkipGram model for Ray Train + MLflow training.

Ported back from the reference `src/model_item2vec/trainer.py` (full MLOps):
  - Ray Train reporting (`ray.train`, `train.report`, `Checkpoint.from_directory`)
    so Tune can collect per-trial val_loss + checkpoints across workers.
  - Evidently classification report generated at the end of training.
  - MLflow logging of the report artifact + classification metrics.
Logs `train_loss` / `val_loss` / `learning_rate` via Lightning + sync_dist for
...
class LitSkipGram(L.LightningModule):
    ...
    def __init__(
        ...
        Args:
            skipgram_model: The SkipGram model instance.
            learning_rate: Learning rate for the optimizer. Defaults to 0.001.
            l2_reg: L2 regularization (weight decay) for the optimizer. Defaults to 1e-5.
            log_dir: Directory to save evaluation reports and logs. Defaults to ".".
        """
        super().__init__()
    ...
    def training_step(
        ...
        Args:
            batch: Batch containing target items, context items, and labels.
    ...
    def configure_optimizers(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing the optimizer and learning rate scheduler configuration.
        """
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        Logs the current learning rate and, in distributed settings, reports the
        validation loss and checkpoint to Ray Train.
        """
    ...
    def _log_classification_metrics(self, val_loader) -> None:
        ...
        Args:
            val_loader: Validation data loader containing batches of target items,
                context items, and labels.
        """
